...
(function(factory){
	...
	var Datepicker = function (target, options) {
		...
		//TRICON: Added minMode for month picker - day, month, year
		if(this.options.minMode=='year'){
			this.startview(2);
		}else if(this.options.minMode=='month'){
			this.startview(1);
		}else{
			...
			case this.keys.space:
				{
					...
					switch (this.gridType) {
						case 0: // days grid
							...
							break;
						case 1: // months grid
							if(this.options.minMode == 'month'){		//TRICON: Added min mode for month picker - day, month, year
								// dismiss the dialog box
								this.month = parseInt($curCell.attr('data-value'), 10);
								$curCell.attr('data-value',1);
								this.update();
								this.hide();
							}
							else{
							...
							break;
						case 2: // years grid
							if(this.options.minMode == 'year'){		//TRICON: Added min mode for month picker - day, month, year
								this.year = parseInt($curCell.attr('data-value'), 10);
								this.month = 0;
								$curCell.attr('data-value',1);
								this.update();
								this.hide();
	...
	Datepicker.prototype.handleGridClick = function(id, e) {
		...
		switch (this.gridType) {
			...
			case 1: // months grid
				if(this.options.minMode == 'month'){		//TRICON: Added min mode for month picker - day, month, year
					// dismiss the dialog box
					this.$grid.attr('aria-activedescendant', $cell.attr('id'));					// update the target box
			...
			case 2: // years grid
				if(this.options.minMode == 'year'){		//TRICON: Added min mode for month picker - day, month, year
					this.$grid.attr('aria-activedescendant', $cell.attr('id'));
					this.year = parseInt($cell.attr('data-value'), 10);
					this.month = 0;
					$cell.attr('data-value',1);
					this.update();
					this.hide();
	...
	Datepicker.prototype.setLocales = function(value) {
		...
		//TRICON: Added min mode for month picker - day, month, year
		this.options.minMode = 'day';
		this.drawCalendarHeader();
		if (this.locales.directionality === 'RTL') {
			this.$grid.addClass('rtl');
		} else {
